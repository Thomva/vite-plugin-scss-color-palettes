  ...
  async function generateScssColorPalettes() {
    ...
    const formatScssVariable = (scssVariables, prefix, formatValueFunction) => {
      return Object.keys(scssVariables).reduce((prev, curr) => {
        ...
        prev[curr.replace(`${prefix}-`, '')] = formatValueFunction(
          scssVariables[curr],
        )
        return prev
      }, {})
    ...
    const colors = formatScssVariable(
      icssExports,
      colorVariablePrefix,
      (value) => value,
    )

    const palettes = Object.keys(colors).map((colorName) => {
      const colorValue = colors[colorName]

      let scssVariablesString = ''
      let scssMapString = `  ${colorName}: (\n`
      ...
      for (let i = 1; i < amount; i++) {
        ...
        switch (true) {
          ...
          case currentStep > 500:
            cssValue = `color-mix(in srgb, ${colorValue}, black ${
              percentage - 100
            }%);`
            break
        ...
        scssVariablesString += `\n$${colorName}-${currentStep}: ${cssValue}`
        scssMapString += `    ${currentStep}: $${colorName}-${currentStep},\n`
      }

      scssMapString += `  ),\n`

      return { colorName, scssVariablesString, scssMapString }
    })

    const scssPalettes = palettes.reduce(
      (prev, curr, index) => {
        prev.scssVariablesString += `${curr.scssVariablesString}\n`
        prev.scssMapString += `${curr.scssMapString}`
        return prev
      },
      { scssVariablesString: '// Color variables', scssMapString: '' },
    )

    // console.log(scssPalettes.scssVariablesString);

    const scssMapString = `\n// Color map\n$color-palettes: (\n${scssPalettes.scssMapString});\n`

    const warning =
      '// This file is generated using the vite-plugin-scss-color-palettes.\n// Do not edit this file directly, changes will be overwritten.\n\n'

    let finalScss = warning + scssPalettes.scssVariablesString

    if (withScssMaps) {
      finalScss += scssMapString
    }
